...
/// <summary>Board list with server-side search + pagination and delete confirmation.</summary>
public partial class Boards : ComponentBase, IDisposable
{
    [Inject] private IOrderHubApiClient Api { get; set; } = null!;

    private List<BoardDto> _items = [];
    ...
    private BoardDto? _pendingDelete;
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            var result = await Api.Boards.SearchAsync(_searchTerm, _pager.Page, _pager.PageSize);
    ...
    private void ConfirmDeleteAsync(BoardDto item) => _pendingDelete = item;
    ...
    private async Task DeleteAsync()
    {
        ...
        try
        {
            await Api.Boards.DeleteAsync(_pendingDelete.Id);
            _pendingDelete = null;
            await LoadAsync();
        }
        catch (Exception ex)
